...
function draw_surface_normals(){
    var success = do_draw_surface_normals();
    if (!success) {
        console.log('Failed to set the positions of the vertices');
        return;
    }
...
function calc_surface_normals_phase1(P_array){
    var P = [];
    var Q = [];
    var R = [];
    var PQ = [];
    ...
    for(var i = 0; i < indexes.length; i += 4){
        ...
        P[i].push(P_array[Pindex - 1]);
        P[i+1].push(P_array[Pindex]);
        P[i+2].pish(P_array[Pindex + 1]);
        Q[i].push(P_array[Qindex - 1]);
        Q[i+1].push(P_array[Qindex]);
        Q[i+2].pish(P_array[Qindex + 1]);
        R[i].push(P_array[Rindex - 1]);
        R[i+1].push(P_array[Rindex]);
        R[i+2].pish(P_array[Rindex + 1]);
    }
...
function calc_surface_normals_phase2(PQ, QR){
    ...
    cross[i].push((PQ[i+1] * QR[i+2]) - (QR[i+1] * PQ[i+2]));
    cross[i+1].push((QR[i] * PQ[i+2]) - (PQ[i] * QR[i+2]));
    cross[i+2].push((PQ[i] * QR[i+1]) - (QR[i] * PQ[i+1]));
    }
...
function calc_surface_normals_phase3(cross){
    for(var i = 0; i < cross.length; i++){
        ...
        norm[i].push(cross[i]/distance);
        norm[i+1].push(cross[i+1]/distance);
        norm[i+2].push(cross[i+2]/distance);
    }
}
...
function do_draw_surface_normals(){
    ...
    var normals_array = new Float32Array(norm);
    ...
    gl.drawArrays(gl.LINE_STRIP, 0, normals_array.length);//Draw lines
    return true;    
}
...
function do_draw_cylinders(){
    ...
    for(var i = 4; i <= index; i += 2){
        ...
        //gl.drawElements(gl.LINE_STRIP, indices_array.length/2, gl.UNSIGNED_SHORT, 0);
     }
    return true;    
}
