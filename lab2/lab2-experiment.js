...
function transformed_cylinder_vertices_part3(P_array, n){
    ...
    //translate to x, y coordinates
     for(var i = 0; i < P_array.length / 2; i += 3){
        ...
        vertices[i + offset] = (P_array[i] * length * cosinetheta) - (P_array[i+1] * sinetheta) + Bx;
        vertices[i+1 + offset] = (P_array[i] * length * sinetheta) + (P_array[i+1] * cosinetheta) + By;
        vertices[i+2 + offset] = P_array[i+2];
    }
      for(var i = 0; i < P_array.length/ 2; i+=3){
        vertices[i] = (P_array[i]  * length * cosinetheta) - (P_array[i+1] * sinetheta) + Ax;
        vertices[i+1] = (P_array[i] * length * sinetheta) + (P_array[i+1] * cosinetheta) + Ay;
...
//function calc_surface_normals(){
//    var vertex = [];
//    for(var i = 0; i < indexes.length; i++) {
//      vertex.push([indexes[i]]);
//      var n = vertex[i][0];
//      //console.log("n: " + n);
//      vertex[i].push([P_array[(n-1) * 3], P_array[((n-1) * 3) +1], P_array[((n-1) * 3) + 2]]);
//    }
//    
